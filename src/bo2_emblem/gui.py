"""
BO2 Emblem Studio - GUI Application
====================================
...
class ShapeListWidget(QTreeWidget):
    ...
    shape_selected = Signal(int)  # shape_id
    ...
    def _populate(self):
        """Populate tree with shape categories and shapes."""
...
class PropertyPanel(QWidget):
    ...
    property_changed = Signal(int, str, object)  # layer_index, property, value
    ...
    def _populate_shapes(self):
        ...
        for cat in CATEGORY_ORDER:
            cat_item = self.shape_combo.model().appendRow()
            cat_item = self.shape_combo.model().item(self.shape_combo.count())
            cat_item = QStandardItem(get_category_display(cat))
    ...
    def set_layer(self, layer: Optional[EmblemLayer]):
        """Set the currently edited layer."""
    ...
    def _block_signals(self, block: bool):
        """Block/unblock signals on all inputs."""
...
class EmblemEditor(QMainWindow):
    ...
    def _on_shape_selected(self, shape_id: int):
        """Add selected shape as new layer."""
        if self.layers:
    ...
    def _on_layers_reordered(self):
        """Reindex layers after drag-drop reorder."""
    ...
    def _generate_ai(self):
        """Generate emblem from AI prompt."""
    ...
    def _optimize(self):
        """Optimize layers to 32 or fewer."""
        if not self.layers:
            return
