...
class EmblemRenderer:
    ...
    def __init__(self, config: Optional[RenderConfig] = None):
        ...
        if self._shapes_dir is None:
            ...
            if hasattr(sys, '_MEIPASS'):
                pkg_dir = os.path.join(sys._MEIPASS, "bo2_emblem", "reference_shapes")
                if os.path.exists(pkg_dir):
                    self._shapes_dir = pkg_dir
                    
            if self._shapes_dir is None:
                # Check research/bo2-emblem-toolkit/reference_shapes
                project_root = os.path.join(os.path.dirname(__file__), "..", "..")
                research_dir = os.path.join(project_root, "research", "bo2-emblem-toolkit", "reference_shapes")
    ...
    def _tinted_shape(self, shape_id: int, r: float, g: float, b: float, a: float) -> Optional[Image.Image]:
        ...
        rgba = Image.merge("RGBA", (
            lum.point(lambda p: int(p * r)),
            lum.point(lambda p: int(p * g)),
            lum.point(lambda p: int(p * b)),
    ...
    def render_png(self, layers: List[EmblemLayer],
        ...
        # Geometry constants (calibrated against live BO2 PC editor)
        # Based on bo2-emblem-toolkit render.py:
        # - A shape at scale=0 (2**0=1) fills the whole preview box
        # - A shape at pos 0.25 lands 0.25*size px from center (so pos 0.5 = the box edge)
        # - position offset in px == pos * size
...
def render_emblem_file(input_path: str, output_path: str, size: int = 512,
    ...
    layers, header = EmblemParser.parse_file(input_path)
